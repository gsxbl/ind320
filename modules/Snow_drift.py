...
def compute_yearly_results(df, T, F, theta):
    ...
    for season, group in df.groupby('season'):
        ...
        result = compute_snow_transport(T, F, theta, swe, wind_speeds)
        result["season"] = f"{season}-{season+1}"
...
def compute_average_sector(df):
    ...
    sectors_list = []
    for season, group in df.groupby('season'):
        # Calculate SWE for this season: precipitation when temperature < 1°C
        winds = group["wind_speed_10m"].tolist()
        dirs = group["wind_direction_10m"].tolist()
        sectors = compute_sector_transport(winds, dirs)
        sectors_list.append(sectors)
    return np.mean(sectors_list, axis=0)

def plot_rose(avg_sector_values, overall_avg):
    """
    Create a polar (wind rose) plot showing the average directional breakdown using Plotly.
    ...
    fig.update_layout(
        title=dict(
            text=f"Average Directional Distribution of Snow Transport<br>"
                 f"<sub>Overall Average Qt: {overall_tonnes:,.1f} tonnes/m</sub>",
            x=0.5,
            xanchor='center'
        ),
        polar=dict(
            radialaxis=dict(visible=True, range=[0, max(values_tonnes) * 1.1]),
...
def main(df=None):
    ...
    df['season'] = df['time'].dt.year - (df['time'].dt.month < 7).astype(int)
    
    # Preprocess: Compute SWE (precipitation when temperature < 1°C)
    df['swe'] = df['precipitation'].where(df['temperature_2m'] < 1, 0)
    ...
    # Prepare yearly summary with tonnes/m conversion
    yearly_df_disp = yearly_df.copy()
    yearly_df_disp["Qt (tonnes/m)"] = yearly_df_disp["Qt (kg/m)"] / 1000
    yearly_summary = yearly_df_disp[['season', 'Qt (tonnes/m)', 'Control']]
    ...
    rose_fig = plot_rose(avg_sectors, overall_avg)
    
    # Compute necessary fence heights for each season and for three fence types.
    ...
    # Return structured results for rendering
    return {
        'overall_avg_tonnes': overall_avg_tonnes,
        'yearly_summary': yearly_summary,
        'rose_fig': rose_fig,
        'fence_results': fence_df
    }
...
if __name__ == "__main__":
    ...
    print(f"Overall average Qt over all seasons: {results['overall_avg_tonnes']:.1f} tonnes/m")
    
    print("\nYearly average snow drift (Qt) per season (in tonnes/m) and control type:")
    print(results['yearly_summary'].to_string(index=False, 
          formatters={'Qt (tonnes/m)': lambda x: f"{x:.1f}"}))
    
    print("\nNecessary fence heights per season (in meters):")
    print(results['fence_results'].to_string(index=False, formatters={
